...
/////////////////////////////////////////////////////////////////////////
        ...
        hWinMain = hinstance;	/* save our main window handle */
...
EchoSocket::EchoSocket(HINSTANCE hinstance)
 {
    /*-------------initialize WinSock DLL------------*/
    this->nRet = WSAStartup(WSA_VERSION, &stWSAData);

    /* WSAStartup() returns error value if failed (0 on success) */
    if (this->nRet != 0)
      WSAperror(this->nRet, "WSAStartup()", hinstance);
      /* No sense continuing if we can't use WinSock */
    else {
      /*---------------release WinSock DLL--------------*/
      nRet = WSACleanup();
      if (nRet == SOCKET_ERROR)
        WSAperror(WSAGetLastError(), "WSACleanup()", hinstance);
         };
...
EchoSocket::~EchoSocket()
 {
             ...
             if (this->hLstnSock != INVALID_SOCKET)
...
int EchoSocket::AcceptConnections()
 {
 ...
 */
 ...
 bool EchoSocket::InitLstnSock(int iLstnPort, PSOCKADDR_IN pstSockName, HWND hWnd, u_int nAsyncMsg)
{
  int nRet;
  SOCKET hLstnSock;
 ...
 */
SOCKET EchoSocket::AcceptConn(SOCKET hLstnSock, PSOCKADDR_IN pstName)
{
  SOCKET hNewSock;
 ...
 */
int EchoSocket::SendData(SOCKET hSock, LPSTR lpOutBuf, int cbTotalToSend)
{
  int cbTotalSent  = 0;
 ...
 */
int EchoSocket::RecvData(SOCKET hSock, LPSTR lpInBuf, int cbTotalToRecv)
{
  int cbTotalRcvd = 0;
 ...
 */
void EchoSocket::DoStats (long lByteCount, long lStartTime, LPCONNDATA lpstSock) {
  LONG dByteRate;
  LONG lMSecs;
 ...
 */
LPCONNDATA EchoSocket::NewConn (SOCKET hSock, PSOCKADDR_IN pstRmtName) {
 ...
 */
LPCONNDATA EchoSocket::FindConn (SOCKET hSock) {
 ...
 */
void EchoSocket::RemoveConn (LPCONNDATA lpstSock) {
  LPCONNDATA lpstSockTmp;
  HLOCAL hSock;
